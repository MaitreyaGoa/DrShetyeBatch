...
function renderQuestion(idx) {
  ...
  if (q.passage) {
    // RC_PASSAGES lookup: always show full passage on every question
    if (window.RC_PASSAGES && window.RC_PASSAGES[q.passage]) {
      var pObj = window.RC_PASSAGES[q.passage];
      html += '<div class="passage-box">'
            + '<div class="passage-label" style="font-weight:700;font-size:0.8rem;margin-bottom:6px;color:#2b6cb0;">📄 '
            + pObj.title + ' — ' + pObj.label + '</div>'
            + '<div style="white-space:pre-line;line-height:1.8;font-size:0.88rem;color:#1a2a3a;">'
            + pObj.text
            + '</div>'
            + '</div>';
    } else {
